...
\begin{tikzpicture}[
  ...
  box/.style={draw=ebay-dark, thick, rounded corners, fill=gray!6,
    text=ebay-dark, font=\small\bfseries, minimum height=9mm, inner xsep=9pt},
  exp/.style={draw=ebay-green, line width=1.1pt, rounded corners, fill=ebay-green!10,
    text=ebay-dark, font=\small\bfseries, minimum height=9mm, inner xsep=9pt},
  outb/.style={draw=ebay-dark, thick, rounded corners, fill=gray!6,
    text=ebay-dark, font=\footnotesize\bfseries, minimum height=7.5mm, inner xsep=7pt},
  capt/.style={font=\scriptsize, text=gray!60!black, align=center, text width=3.15cm},
  clbl/.style={font=\scriptsize\bfseries, text=ebay-green, align=center},
  ann/.style={font=\scriptsize\itshape, text=ebay-dark},
  arr/.style={->, thick, draw=ebay-dark}
]
\node[box] (hist) {Code history};
\node[box, right=11mm of hist] (graphs) {Family of graphs};
\node[exp, right=13mm of graphs] (exp) {Retrieval experiment};
\node[outb, right=14mm of exp, yshift=8mm]  (synth) {Cross-repo synthesis};
\node[outb, right=14mm of exp, yshift=-8mm] (valid) {Convention validation};
\node[capt, below=2mm of hist]   {commits $\cdot$ PRs $\cdot$ JIRA $\cdot$ code patterns $\cdot$ external resources};
\node[capt, below=2mm of graphs] {Repo $\cdot$ JIRA $\cdot$ Memory-Bank; time-weighted, multi-resolution};
\node[capt, below=2mm of exp]    {3 strategies: graph query $\cdot$ similarity/RAG $\cdot$ spreading activation};
\node[clbl, above=2mm of exp] {CORE CONTRIBUTION};
\draw[arr] (hist)   -- (graphs);
\draw[arr] (graphs) -- (exp);
\draw[arr] (exp.east) -- node[ann, above, pos=0.62]{best} (synth.west);
\draw[arr] (exp.east) -- (valid.west);
\node[ann, below=15mm of graphs, xshift=2.4cm, text width=10cm, align=center]
  {e.g., adding a ``selling fast'' signal to the ViewItem page spans 5+ services.};
\end{tikzpicture}%
}
...
\textbf{Metrics for success}, ordered to match where the effort goes: first, the \emph{retrieval-strategy comparison} itself---retrieval precision/recall and cross-repository feature-scaffold accuracy across the three strategies, isolating which is most efficient (the core result); then the \emph{applied outcome} that motivates it---reduction in review-cycle time and reviewer effort per agent-generated PR; and, supporting, onboarding-task success. These are reported with statistics appropriate to a controlled comparison: paired significance tests across the three strategies (Friedman, with post-hoc Wilcoxon signed-rank and multiple-comparison correction), effect sizes (Cliff's $\delta$) reported alongside $p$-values, bootstrap 95\% confidence intervals on every delta, and inter-rater agreement (Krippendorff's $\alpha$) for human-judged scaffold quality. Tiered funding could support progressive expansion to additional repositories and pattern types across eBay's organization.
...
\small
[1] J.~Cusati and C.~Brown. ``From Papers to Progress: Rethinking Knowledge Accumulation in Software Engineering.'' In \textit{Proc.\ ICSE-FOSE}, arXiv:2604.16208, 2026. 
...
[8] T.~M.~N.~Nimraka et al. ``An Agentic-AI Solution for Intelligent Code Review.'' In \textit{Proc.\ IEEE SLAAI-ICAI}, 2025.
...
[10] M.~Janke and P.~M\"ader. ``Graph Based Mining of Code Change Patterns From Version Control Commits.'' In \textit{IEEE Trans.\ Softw.\ Eng.}, 2022.

[11] M.~Dilhara et al. ``Unprecedented Code Change Automation: The Fusion of LLMs and Transformation by Example.'' In \textit{Proc.\ ACM Softw.\ Eng.} (FSE), 2024.
...
[21] J.~Pavlovi\'c et al. ``Leveraging Spreading Activation for Improved Document Retrieval in Knowledge-Graph-Based RAG Systems.'' arXiv:2512.15922, 2025.
